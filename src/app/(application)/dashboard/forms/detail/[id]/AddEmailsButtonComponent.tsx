...
import { z } from 'zod'
import { addEmailsGroupScheduleSender } from './actions'
...
function AddEmailsButtonComponent({groupId, isArchived}: {groupId: string, isArchived: boolean}) {
    ...
    const onSubmit = async (data: EditEmailsToGroupFormSchema) => {
        try {
          ...
          const updatedGroup = await addEmailsGroupScheduleSender(data);
    
          if(updatedGroup){
            toast({
              title: "Client email(s) have been added to survey!",
              description: `Scheduled ${updatedGroup.activeEmailCount} client(s) to be emailed!`,
              variant: "destructive",
          ...
          toast({
            title: "Could not add client emails to survey group!",
  ...
  return (
    <Popover>
        <PopoverTrigger asChild>
            <Button disabled={isArchived} className="disabled:cursor-not-allowed text-black py-0 gap-1"><Icons.add height={15} width={15}/> Emails</Button>
        </PopoverTrigger>
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="w-full space-y-6">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Add Emails</FormLabel>
                        <FormControl>
                          <Textarea
...
export default AddEmailsButtonComponent